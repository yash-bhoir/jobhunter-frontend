...
export default function Search() {
  ...
  useEffect(() => {
    ...
    api.get('/search/history?limit=5').then(({ data }) => setHistory(data.data || [])).catch(() => {}).finally(() => setHistoryLoading(false));
  }, []);
  ...
  const handleSearch = async (force = false) => {
    ...
    try {
      ...
      api.get('/search/history?limit=5').then(({ data: h }) => setHistory(h.data || [])).catch(() => {});
  ...
  return (
    <div className="max-w-xl mx-auto py-2">
      <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-4">
        ...
        {/* ── Header ─────────────────────────────────────────── */}
        <motion.div variants={fadeUp} className="text-center pb-1">
          ...
          <p className="text-sm text-gray-400 mt-1">
            Search {userPlatforms.length} platforms · Auto-find HR emails · AI outreach
          </p>
        </motion.div>
          ...
          <div className="p-5 space-y-4">
                      ...
                      {userPlatforms.map((platform) => {
                        ...
                        return (
                          <button
                            ...
                            disabled={locked}
                            className={cn(
                              'flex items-center gap-2 px-3 py-2 rounded-lg border text-left transition-all duration-150 text-sm',
                          ...
                          >
                            ...
                            {locked && <span className="ml-auto text-[10px] bg-amber-100 text-amber-600 font-semibold px-1.5 py-0.5 rounded">Pro</span>}
                          </button>
                        );
                      })}
                    </div>
                ...
                >
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <p className="text-xs font-semibold text-emerald-800">
                      {cacheInfo.jobCount} cached jobs · {cacheInfo.ageDays === 0 ? 'today' : `${cacheInfo.ageDays}d ago`}
                    </p>
                  </div>
            ...
            {!isPro && (
              <p className="text-[11px] text-center text-gray-400">
                ...
                <a href="/billing" className="text-blue-600 font-semibold hover:underline">Upgrade</a> for all
              </p>
            )}
          </div>
        </motion.div>
            ...
            >
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-xl bg-blue-50 flex items-center justify-center">
                  <Zap className="w-4.5 h-4.5 text-blue-600 animate-pulse" style={{width:'18px',height:'18px'}} />
                </div>
                <div>
              ...
              <div className="pt-3 border-t border-gray-100 flex items-center gap-2">
                ...
                {emailProgress ? (
                  ...
                  <p className="text-xs text-gray-400">HR email search queued…</p>
                )}
              </div>
            </motion.div>
          )}
          ...
          >
            ...
            {historyLoading ? (
              <div className="p-3 space-y-2">
                {[...Array(3)].map((_, i) => <div key={i} className="skeleton h-9 rounded-lg" />)}
              </div>
            ) : (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-gray-400">
                          ...
                          {s.totalFound > 0 && <span className="ml-2 text-emerald-500 font-medium">{s.totalFound} jobs</span>}
                        </p>
                      </div>
